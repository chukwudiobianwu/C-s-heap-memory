...
{
	event_t *slum = (event_t *)emalloc(sizeof(event_t));
	
	slum->repetition = event->repetition;

	strncpy(slum->description, event->description, strlen(event->description));

	strncpy(slum->location, event->location, strlen(event->location));

        strncpy(slum->end_date, event->end_date, strlen(event->end_date));

	strncpy(slum->start_date, event->start_date, strlen(event->start_date));
